...
import { summarizeReviewSnippet } from '@/lib/reviews'
...
export const summarizeReviewsForPrompt = (reviews: readonly TrustpilotReview[]) =>
  JSON.stringify(
    {
      reviewCount: reviews.length,
      averageRating: toAverageRating(reviews),
      reviews: reviews.map(summarizeReviewSnippet),
    },
    null,
    2,
  )
...
export const getSystemPrompt = (mode: AnalystMode, reviews: readonly TrustpilotReview[]) => {
  const reviewsJson = summarizeReviewsForPrompt(reviews)
  const template = mode === 'exec' ? kSystemPromptExec : kSystemPromptAnalyst
  return template.replace('{REVIEWS_JSON}', reviewsJson)
}
