...
export const BChatPanel: FC<BChatPanelProps> = ({
  isSidebar = false,
}) => {
      ...
      >
        ...
        <div className="relative mb-5 shrink-0 pr-16">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.34em] text-black/30">Conversation</p>
              <h2 className="mt-2 text-[40px] font-semibold tracking-[-0.04em] text-black">Ask the dataset</h2>
            </div>
        ...
        <div className="relative min-h-0 flex-1 overflow-hidden rounded-[34px] border border-black/8 bg-white/78 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)] backdrop-blur-sm">
          ...
          <div className="flex h-full min-h-0 flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 pr-5" ref={messageViewportRef}>
              <BMessageList
                isCompact
  ...
  return (
    ...
    >
      ...
      <div className={`mb-6 flex shrink-0 flex-col gap-4 ${isSidebar ? '' : 'xl:flex-row xl:items-center xl:justify-between'}`}>
        <div>
          <p className="text-sm uppercase tracking-[0.28em] text-black/35">Conversation</p>
          <h2 className={`mt-1 font-semibold tracking-tight text-black ${isSidebar ? 'text-3xl' : 'text-4xl'}`}>Ask the dataset</h2>
