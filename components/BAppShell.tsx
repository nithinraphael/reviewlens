...
const isTrustpilotReviewUrl = (value: string) =>
  value.includes("trustpilot.com/review/");

const toDisplayRating = (rating: number) =>
    ...
    <div className="relative overflow-hidden rounded-[26px] border border-black/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.86)_0%,rgba(255,252,245,0.92)_100%)] px-5 py-5 backdrop-blur-sm lg:px-6 lg:py-6">
      ...
      <div className="relative grid gap-6 xl:grid-cols-[0.92fr_1.08fr] xl:items-center">
        <div className="max-w-xl">
          ...
          <p className="mt-4 max-w-[34rem] text-[16px] leading-7 text-black/60 lg:text-[18px] lg:leading-8">
            {body}
          </p>
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-3">
            {kIdleStatCards.map(
              (
                { title: statTitle, value, body: statBody, className },
                index,
              ) => (
                <motion.div
                ...
                >
                  <div className="text-[10px] uppercase tracking-[0.24em] text-black/38">
                    {statTitle}
                  </div>
                  <div className="mt-3 text-[22px] font-semibold tracking-[-0.05em] text-black">
                    {value}
                  </div>
                  <div className="mt-2 text-[13px] leading-6 text-black/58">
                    {statBody}
                  </div>
                </motion.div>
              ),
        ...
        <div className="relative min-h-[340px]">
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-3 text-[34px] font-semibold tracking-[-0.06em]">
                  {accentLabel}
                </div>
              </div>
          ...
          >
            <div className="text-[10px] uppercase tracking-[0.24em] text-black/34">
              Mood index
            </div>
...
export const BAppShell: FC = () => {
  ...
  const { ingest, error } = useReviewIngest();
  ...
  const [navbarUrl, setNavbarUrl] = useState(storedUrl);
  ...
  useEffect(() => {
    ...
    const visibleSections = sections.filter(
      (
        section,
      ): section is ObservedSection & { readonly element: HTMLElement } =>
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        const match = visibleSections.find(
          ({ element }) => element === visible.target,
        );
  ...
  const handleNavbarUrlChange = (event: ChangeEvent<HTMLInputElement>) => {
    setNavbarUrl(event.target.value);
  };

  const handleNavbarSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();

    const trimmedUrl = navbarUrl.trim();
    if (!isTrustpilotReviewUrl(trimmedUrl)) {
      ...
      <div className="flex h-full w-full overflow-hidden bg-[#fbfaf7]">
        <aside className="relative hidden h-full w-[280px] shrink-0 overflow-hidden border-r border-black/8 bg-[linear-gradient(180deg,#ffffff_0%,#fcfaf5_100%)] lg:flex lg:flex-col">
          ...
          <div className="border-b border-black/8 px-8 py-8">
            <div className="text-[11px] uppercase tracking-[0.42em] text-black/35">
          ...
          <div className="flex min-h-0 flex-1 flex-col justify-between px-4 py-5">
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-[11px] uppercase tracking-[0.34em] text-black/32">
                    Tutorial
                  </div>
                  <div className="mt-3 text-[22px] font-medium tracking-tight text-black">
                    See how it works
                  </div>
                  <p className="mt-2 text-[14px] leading-6 text-black/56">
                    Animated walkthrough of the ingest, brief, chat, and export
                    flow.
                  </p>
                </div>
                <motion.div
        ...
        <section className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-[#fbfaf7]">
                ...
                {hasStoredData ? (
                  ...
                  >
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                      <input
                        className="h-11 w-full rounded-full border border-black/12 bg-white px-4 text-sm text-black/90 outline-none transition-[border-color,box-shadow] duration-200 placeholder:text-black/38 focus:border-black/30 focus:shadow-[0_8px_20px_rgba(18,18,18,0.08)]"
                        onChange={handleNavbarUrlChange}
                        placeholder="Paste a new Trustpilot URL for analysis"
                        value={navbarUrl}
                      />
                      <motion.button
                        className="h-11 shrink-0 rounded-full border border-black bg-[#121212] px-5 text-sm font-semibold text-white shadow-[0_12px_28px_rgba(18,18,18,0.26)] transition-[transform,box-shadow,background-color] duration-200 hover:bg-black disabled:cursor-not-allowed disabled:border-black/30 disabled:bg-black/35 disabled:text-white/75"
                        disabled={isLoading}
                        type="submit"
                        whileHover={
                          isLoading ? undefined : { y: -2, scale: 1.01 }
                        }
                        whileTap={isLoading ? undefined : { scale: 0.98 }}
                      >
                        {isLoading ? "Analyzing..." : "Analyze new URL"}
                      </motion.button>
                    </div>
                  </form>
                ) : (
          ...
          <div className="min-h-0 flex-1 overflow-hidden px-6 py-7 lg:px-10">
            <div
              className="h-full min-h-0 min-w-0 overflow-y-auto overflow-x-hidden pr-1"
              ref={contentRef}
            >
                  ...
                  {reviews.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="mb-5 flex items-center justify-between gap-3">
                        ...
                        <div className="bg-[#e8f255] px-3 py-1 text-sm">
                          {reviews.length}
                        </div>
                      </div>
